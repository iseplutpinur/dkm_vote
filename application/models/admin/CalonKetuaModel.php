...
class CalonKetuaModel extends Render_Model
{
    ...
    {
        if ($id == null) {
            $cek = $this->db->get_where("kpu_calon", ["status" => 0]);
            $this->db->trans_start();
            if ($cek->num_rows() == 0) {
                $this->db->insert("kpu_calon", [
                    "status" => 0
                ]);
            ...
            $return = $this->db->select('*')->from('kpu_calon')->where('id', $getID)->get()->row_array();
            $this->db->trans_complete();

            return $return;
        } else {
            $id = $this->db->select('*')->from('kpu_calon')->where('id', $id)->get()->row_array();
            return $id;
        }
    }
